...
| **Android WebView app** (own browser/reader app) | `shouldInterceptRequest` + Rust via JNI/UniFFI. But the hook is synchronous, has **no initiator and no resource type** → URL-only matching, degraded third-party logic; cosmetics via `evaluateJavascript`. No established open-source example found | Partial |
