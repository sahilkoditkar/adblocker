...
The standard objection is "`shouldInterceptRequest` only gives you a URL." In practice `WebResourceRequest.getRequestHeaders()` exposes Chromium's real headers, including **`Sec-Fetch-Dest`** (image/script/style/iframe/font/…) and **`Referer`** — which reconstructs exactly the two extra inputs `Request::new()` needs: `request_type` from `Sec-Fetch-Dest`, `source_url` from `Referer` (+`isForMainFrame()`).
